...
const postItem = (req, res) => {
  ...
  req
    ...
    .on('end', () => {
      ...
      const id =
        items.reduce((max, item) => (item.id > max ? item.id : max), 0) + 1; // item id is the highest id + 1
      item.id = id;
...
const getItems = (req, res) => {
  // search for items by name or get all items
  let body = [];
  req
    .on('data', (chunk) => {
      body.push(chunk);
    ...
    .on('end', () => {
      ...
      try {
        ...
        res.end(
          JSON.stringify({
            error: '400',
            message: ' Invalid request, no item found',
          }),
        );
      }
    });
